...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "Javaee-Practico-ear.ear");

        // Current directory is the root of the "Javaee-Practico-web" project. Go up one level
        // and enter the "Javaee-Practico-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../Javaee-Practico-ejb/target/Javaee-Practico-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "Javaee-Practico-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/Javaee-Practico-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "Javaee-Practico-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
